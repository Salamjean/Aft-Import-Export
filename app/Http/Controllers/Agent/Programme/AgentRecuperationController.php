...
class AgentRecuperationController extends Controller
{
    public function index()
    {
        ...
        // Filtrer les récupérations par l'agence du chauffeur
        ...
        $totalRecuperations = Recuperation::whereHas('chauffeur', function($query) use ($agenceId) {
            $query->where('agence_id', $agenceId);
        })->count();
        
        $programmesCount = Recuperation::whereHas('chauffeur', function($query) use ($agenceId) {
            $query->where('agence_id', $agenceId);
        })->where('statut', 'programme')->count();
        
        $enCoursCount = Recuperation::whereHas('chauffeur', function($query) use ($agenceId) {
            $query->where('agence_id', $agenceId);
        })->where('statut', 'en_cours')->count();
        
        $terminesCount = Recuperation::whereHas('chauffeur', function($query) use ($agenceId) {
            $query->where('agence_id', $agenceId);
        })->where('statut', 'termine')->count();
        
        return view('agent.recuperation.index', compact(
            'recuperations', 
            'chauffeurs',
            'totalRecuperations',
            'programmesCount',
    ...
    public function create()
    {
        ...
        // Filtrer les chauffeurs par l'agence de l'agent
        $chauffeurs = Chauffeur::where('agence_id', $agenceId)->get();
        return view('agent.recuperation.create', compact('chauffeurs'));
    }
        ...
        try {
            ...
            return response()->json([
                ...
                'data' => [
                    ...
                    'nom_client' => $recuperation->nom_client,
                    'prenom_client' => $recuperation->prenom_client,
                    'contact_client' => $recuperation->contact_client,
                    'email_client' => $recuperation->email_client,
                    'codes_qr' => $codesQr,
    ...
    public function etiquettes($id)
    {
        try {
            ...
            return view('agent.recuperation.etiquette', compact('recuperation', 'codes'));
            
        } catch (\Exception $e) {
            return response()->view('agent.recuperation.etiquette', [
                'recuperation' => null,
                'codes' => [],
    ...
    public function downloadEtiquettes($id)
    {
        try {
            ...
            $pdf = PDF::loadView('agent.recuperation.etiquette', compact('recuperation', 'codes'))
    ...
    public function destroy($id)
    {
        try {
            ...
            if (!$recuperation) {
                ...
                return redirect()->route('agent.recuperation.index')->with('error', 'Récupération non trouvée.');
            }
