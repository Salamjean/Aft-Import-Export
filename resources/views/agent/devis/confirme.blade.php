                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h4 class="card-title mb-0">
                            ...
                            Demandes de Devis Confirmés
                        </h4>
                        <div class="d-flex align-items-center">
                            <span class="badge badge-warning me-3">
                                <i class="fas fa-clock me-1"></i>
                                {{ $devisEnAttente->total() }} demande(s) confirmé(s)
                            </span>
                        </div>
                    </div>
                    <div class="card-body">
                        ...
                        @if ($devisEnAttente->count() > 0)
                            <div class="table-responsive">
                                <table class="table table-hover">
                                    <thead>
                                        <tr>
                                            <th class="text-center text-white">Réf Enlevement</th>
                                            ...
                                            <th class="text-center text-white">Agence Expédition</th>
                                            <th class="text-center text-white">Nombre Colis</th>
                                            <th class="text-center text-white">Valeur Totale</th>
                                            <th class="text-center text-white">Montant Proposé</th>
                                    ...
                                    <tbody>
                                        @foreach ($devisEnAttente as $devi)
                                            <tr
                                                class="{{ $devi->created_at->diffInHours(now()) < 24 ? 'new-request' : '' }}">
                                                <td class="text-center">{{ $devi->reference_devis ?? 'Envoie en agence' }}
                                                </td>
                                                <td class="text-center">
                                                    <div class="d-flex flex-column">
                                                        <span
                                                            class="font-weight-bold">{{ $devi->created_at->format('d/m/Y') }}</span>
                                                        <small
                                                            class="text-muted">{{ $devi->created_at->format('H:i') }}</small>
                                                        @if ($devi->created_at->diffInHours(now()) < 24)
                                                            <small class="text-warning"><i
                                                                    class="fas fa-star me-1"></i>Nouveau</small>
                                                        @endif
                                                    </div>
                                                </td>
                                                <td class="text-center">
                                                ...
                                                <td class="text-center">
                                                    <strong class="text-success">
                                                        {{ number_format($valeurTotale, 0, ',', ' ') }}
                                                        {{ $devi->devise }}
                                                    </strong>
                                                </td>
                                                <td class="text-center">
                                                    <strong class="text-success">
                                                        {{ number_format($devi->montant_devis, 0, ',', ' ') }}
                                                        {{ $devi->devise }}
                                                    </strong>
                                                </td>
                                                <td class="text-center">
                                                    <span class="badge badge-success status-badge">
                                                        ...
                                                        {{ $devi->statut == 'traite' ? 'Validé' : 'Confirmé' }}
                                                    </span>
                                                </td>
                                                </td>
                                            </tr>
                ...
                .then(devis => {
                    ...
                    if (devis.colis && Array.isArray(devis.colis)) {
                        devis.colis.forEach((colis, index) => {
                            ...
                            const photoHtml = colis.photo ? `
                        <a href="/storage/${colis.photo}" target="_blank">
                            <img src="/storage/${colis.photo}" alt="Colis #${index + 1}" style="max-height: 40px; border-radius: 4px; border: 1px solid #ddd; padding: 1px;" />
                        </a>
                    ` : 'N/A';
                                        ...
                                        <table class="table table-sm table-bordered">
                                            ...
                                            <tfoot>
                                                <tr>
                                                    ...
                                                    <td colspan="5"><strong>${totalValeur.toLocaleString('fr-FR', {minimumFractionDigits: 0, maximumFractionDigits: 0})} ${devis.devise}</strong></td>
                                                </tr>
                                            </tfoot>
                                        </table>
                                    </div>
