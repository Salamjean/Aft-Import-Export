...
@section('content')
            ...
            <div class="col-12">
                <div class="card modern-card">
                    <div class="card-header modern-header">
                        <div class="header-content">
                            ...
                            <div class="header-text">
                                <h3 class="card-title">Déchargement des Colis</h3>
                                <p class="card-subtitle">Scanner les colis pour les décharger des conteneurs</p>
                            </div>
                        </div>
                        <div class="header-actions">
                            <button onclick="openQRScannerDecharge()" class="btn modern-btn text-white"
                                style="background-color:#17a2b8; margin-right: 10px;">
                                <i class="fas fa-qrcode"></i>
                                Scanner pour Déchargement
                            </button>
                            <a href="{{ route('agent.scan.charge') }}" class="btn modern-btn"
                    ...
                    <div class="card-body">
                        ...
                        <form method="GET" action="{{ route('agent.scan.decharge') }}" class="table-controls">
                            <div class="row">
                                <div class="col-md-4">
                                ...
                                <div class="col-md-8">
                                    <div class="filter-group">
                                        ...
                                        <a href="{{ route('agent.scan.decharge') }}" class="btn btn-secondary">
                                ...
                                <tbody>
                                    @forelse($colis as $item)
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <!-- Barre de progression pour le déchargement -->
                                                @if ($item->total_individuels > 0)
                                                    @php
                                                        $pourcentageDecharge =
                                                            ($item->individuels_decharges / $item->total_individuels) *
                                                            100;
                                                    @endphp
                                                    <div class="progress mt-2" style="height: 20px;">
                                                        <div class="progress-bar bg-primary" role="progressbar"
                                                            style="width: {{ $pourcentageDecharge }}%"
                                                            aria-valuenow="{{ $pourcentageDecharge }}" aria-valuemin="0"
                                                            aria-valuemax="100">
                                                            {{ $item->individuels_decharges }}/{{ $item->total_individuels }}
                                                        </div>
                                                    </div>
                                                    <small class="text-muted">
                                                        {{ number_format($pourcentageDecharge, 1) }}% déchargés
                                                    </small>
                                                @else
                                        ...
                                        <tr>
                                            <td colspan="11" class="text-center">
                                                <div class="no-data-content py-5">
                                                    ...
                                                    <p class="text-muted">Aucun colis disponible pour le déchargement</p>
                                                </div>
                                            </td>
                                        </tr>
                                    @endforelse
    ...
    <script>
        ...
        // Fonction pour ouvrir le scanner QR code pour déchargement
        function openQRScannerDecharge() {
            console.log("🔓 Ouverture du scanner QR pour déchargement...");

            Swal.fire({
                title: '📦 Scanner pour Déchargement',
                html: `
                <div class="text-center">
                    <div class="alert alert-info">
                        <i class="fas fa-info-circle"></i>
                        Scanner les unités qui sont dans les conteneurs pour les décharger
                    </div>
                    <div class="mb-3">
                        <label for="agenceSelect" class="form-label">Agence de Destination (Optionnel)</label>
                        <select class="form-control" id="agenceSelect">
                    ...
                    <div id="qr-scanner-container" style="width: 100%; max-width: 500px; margin: 0 auto;">
                        <div id="qr-reader" style="width: 100%; min-height: 300px; border: 2px dashed #17a2b8; border-radius: 10px; background: #f8f9fa;"></div>
                    </div>
                    <div class="mt-3">
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    <div class="col-md-4">
                                        <button type="button" class="btn btn-outline-primary w-100" onclick="validateManualCodeDecharge()">
                                            Valider
                                        </button>
                                    </div>
                                </div>
        ...
        // Démarrer le scanner QR pour déchargement
        function startQRScannerDecharge() {
            console.log("🚀 Démarrage du scanner déchargement...");
            ...
            try {
                ...
                // Vider le conteneur
                container.innerHTML = `
                <div id="qr-reader" style="width: 100%; min-height: 300px; border: 2px dashed #17a2b8; border-radius: 10px; background: #f8f9fa;">
        ...
        // Callback quand un QR code est détecté pour déchargement
        function onScanSuccessDecharge(decodedText, decodedResult) {
            console.log("🎯 QR Code détecté pour déchargement:", decodedText);
            ...
            handleScannedQRCodeDecharge(decodedText);
        }

        // Fonction pour traiter le QR code scanné pour déchargement
        function handleScannedQRCodeDecharge(qrCode) {
            console.log("🔄 Traitement du QR code pour déchargement:", qrCode);

            // Récupérer l'ID de l'agence sélectionnée
            const agenceSelect = document.getElementById('agenceSelect');
            const agenceId = agenceSelect ? agenceSelect.value : null;
            ...
            stopQRScanner().then(() => {
                ...
                showProcessingModalDecharge(qrCode, agenceId);
        ...
        // Afficher le modal de traitement pour déchargement
        function showProcessingModalDecharge(qrCode, agenceId) {
            Swal.fire({
                title: '🔄 Déchargement en cours...',
                html: `
                <div class="text-center">
                    ...
                    <div class="mt-3 p-3 bg-light rounded">
                        ...
                        ${agenceId ? `<br><strong>Agence:</strong> #${agenceId}` : ''}
                    </div>
                    <p class="mt-3">Déchargement de l'unité vers l'agence...</p>
                </div>
            `,
                showConfirmButton: false,
                ...
                didOpen: () => {
                    ...
                    processScannedCodeDecharge(qrCode, agenceId);
                }
            });
        }

        // Fonction pour traiter le code scanné avec le serveur pour déchargement
        function processScannedCodeDecharge(qrCode, agenceId) {
            console.log("📡 Envoi au serveur pour déchargement:", qrCode, agenceId);

            const url = '/agent/scan/scan-qr-decharge';
            ...
            fetch(url, {
                ...
                body: JSON.stringify({
                    qr_code: qrCode,
                    agence_destination_id: agenceId
                })
            })
                .then(response => {
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        showScanSuccessDecharge(data, qrCode);
                    } else {
                        showScanErrorDecharge(data, qrCode);
                    }
                })
                .catch(error => {
                    console.error('❌ Erreur complète:', error);
                    showScanConnectionErrorDecharge(error, qrCode);
                });
        }

        // Afficher le succès du scan pour déchargement
        function showScanSuccessDecharge(data, qrCode) {
            Swal.fire({
                ...
                <div class="text-center">
                    ...
                    ${data.unite ? `
                        <div class="alert alert-success">
                            ...
                            <p class="mb-1"><strong>Statut:</strong> 
                                <span class="badge bg-primary">${data.unite.nouveau_statut}</span>
                            </p>
                    ...
                    ${data.colis ? `
                        <div class="alert alert-info">
                            <h6>📊 Progression du déchargement</h6>
                            <p class="mb-1"><strong>Référence:</strong> ${data.colis.reference_colis}</p>
                            <p class="mb-1"><strong>Progression:</strong> 
                                ${data.colis.unites_dechargees}/${data.colis.total_unites} unités déchargées
                            </p>
        ...
        // Afficher les erreurs pour le déchargement
        function showScanErrorDecharge(data, qrCode) {
            Swal.fire({
        ...
        // Afficher les erreurs de connexion pour déchargement
        function showScanConnectionErrorDecharge(error, qrCode) {
            Swal.fire({
                title: '❌ Erreur de Connexion',
                html: `
                <div class="text-center">
                    <i class="fas fa-exclamation-triangle text-danger" style="font-size: 3rem;"></i>
                    <p class="mt-3"><strong>Erreur lors du déchargement</strong></p>
                    <p class="text-muted">${error.message}</p>
                    <div class="mt-3">
                        <button class="btn btn-primary me-2" onclick="openQRScannerDecharge()">
        ...
        // Entrée manuelle depuis le scanner pour déchargement
        function validateManualCodeDecharge() {
            const manualInput = document.getElementById('manualCodeInput');
            ...
            Swal.close();
            processScannedCodeDecharge(code, agenceId);
        }
                ...
                preConfirm: () => {
                    ...
                    if (montant > montantRestant) {
                        Swal.showValidationMessage(
                            `Le montant ne peut pas dépasser ${montantRestant.toFixed(2)} ${devise}`);
                        return false;
                    }
        ...
        function openManualInput() {
            ...
            `,
                ...
                if (result.isConfirmed) {
                    processScannedCodeDecharge(result.value, null);
                }
            });
        }
        ...
        console.log("✅ Page de déchargement des colis chargée avec toutes les fonctionnalités");
    </script>
@endsection