        ...
        <div class="col-12">
            <div class="card modern-card">
                ...
                <div class="header-actions">
                    <button onclick="openQRScanner()" class="btn modern-btn text-white" style="background-color:#28a745; margin-right: 10px;">
                ...
                <div class="card-body">
                    ...
                    <form method="GET" action="{{ route('agent.scan.entrepot') }}" class="table-controls">
                        <div class="row">
                            <div class="col-md-4">
                            ...
                            <div class="col-md-8">
                                <div class="filter-group">
                                    ...
                                    <a href="{{ route('agent.scan.entrepot') }}" class="btn btn-secondary">
    ...
    @endif
                                    ...
                                    <td class="text-center">
                                        <div class="action-buttons">
                                            ...
                                            <a href="{{ route('agent.colis.edit', $item->id) }}" class="btn-action btn-edit" title="Modifier">
...
// Fonction pour ouvrir le scanner QR code
        ...
        html: `
            <div class="text-center">
                <div class="alert alert-info">
                    ...
                    Autorisez l'accès à votre caméra. Pointez vers un QR code.
                </div>
                <div id="qr-scanner-container" style="width: 100%; max-width: 500px; margin: 0 auto;">
                    <div id="qr-reader" style="width: 100%; min-height: 300px; border: 2px dashed #28a745; border-radius: 10px; background: #f8f9fa;"></div>
                </div>
                <div class="mt-3">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <button type="button" class="btn btn-outline-primary w-100" onclick="validateManualCode()">
                                        Valider
                                    </button>
                                </div>
                            </div>
...
function startQRScanner() {
    ...
    try {
        ...
        // Vider le conteneur
        container.innerHTML = `
            <div id="qr-reader" style="width: 100%; min-height: 300px; border: 2px dashed #28a745; border-radius: 10px; background: #f8f9fa;">
...
// Fonction pour traiter le QR code scanné
function handleScannedQRCode(qrCode) {
    console.log("🔄 Traitement du QR code:", qrCode);
...
function showProcessingModal(qrCode) {
    Swal.fire({
        ...
        html: `
            <div class="text-center">
                ...
                <p class="mt-3">Validation avec le serveur...</p>
            </div>
        `,
        showConfirmButton: false,
...
// Fonction pour traiter le code scanné avec le serveur
function processScannedCode(qrCode) {
    console.log("📡 Envoi au serveur:", qrCode);

    // URL directe - NE PAS utiliser route() pour éviter les problèmes
    const url = '/agent/scan/scan-qr';
    ...
    .then(data => {
        ...
        if (data.success) {
            showScanSuccess(data, qrCode);
        } else {
            showScanError(data, qrCode);
        }
    })
    .catch(error => {
        console.error('❌ Erreur complète:', error);
        showScanConnectionError(error, qrCode);
    });
}

// Afficher le succès du scan
function showScanSuccess(data, qrCode) {
    Swal.fire({
        title: '✅ Succès !',
        html: `
            <div class="text-center">
...
function showScanError(data, qrCode) {
    Swal.fire({
...
function showScanConnectionError(error, qrCode) {
    Swal.fire({
        title: '❌ Code Incorrect',
        html: `
            <div class="text-center">
                <i class="fas fa-exclamation-triangle text-danger" style="font-size: 3rem;"></i>
                <p class="mt-3"><strong>Code incorrect ou invalide</strong></p>
                <div class="mt-3">
                    <button class="btn btn-primary me-2" onclick="openQRScanner()">
                        <i class="fas fa-redo"></i> Ressayer
                    </button>
                    <button class="btn btn-outline-secondary" onclick="openManualInput()">
...
// Entrée manuelle depuis le scanner
...
function openManualInput() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            processScannedCode(result.value);
        }
    });
}
...
// Test de l'API depuis la console
function testScanAPI() {
    const testCode = "CO-3HAJVMZS";
    console.log("🧪 Test API avec:", testCode);
    processScannedCode(testCode);
}

console.log("✅ Scanner QR code chargé - Utilisez openQRScanner() pour démarrer");
</script>
<script>
...
function downloadAllColisPDF() {
    ...
    let url = '/agent/parcel/export/pdf?' + searchParams.toString();
...
<style>
/* Styles pour les badges de statuts individuels */
.statuts-individuels .badge {
    font-size: 8px;
    padding: 2px 4px;
    border-radius: 3px;
    font-weight: 600;
