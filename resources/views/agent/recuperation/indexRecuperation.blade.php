...
async function showDetails(demandeId) {
    ...
    try {
        ...
        const url = `{{ route('agent.demandes-recuperation.details', ':id') }}`.replace(':id', demandeId);
...
async function markAsTraite(demandeId) {
    ...
    try {
        const response = await fetch(`/agent/request/${demandeId}/traiter`, {
            method: 'POST',
            headers: {
...
async function markAsAnnule(demandeId) {
    ...
    try {
        const response = await fetch(`/agent/request/${demandeId}/annuler`, {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json',
