                            ...
                            <tr data-id="{{ $planification->id }}" 
                                ...
                                <td class="text-center">
                                    <div class="btn-group btn-group-sm">
                                        <a href="{{ route('agent.cote.bateau.conteneur', $planification->id) }}" class="btn btn-action btn-open me-1" title="Ouvrir le bateau">
                                            <i class="fas fa-door-open"></i>
                                        </a>
                                        @if($planification->statut !== 'arrive')
                                        <button class="btn btn-action btn-arrived" 
                                                onclick="markAsArrived(this)" 
                                                data-url="{{ route('agent.bateaux.mark-arrived', $planification->id) }}"
                                                data-id="{{ $planification->id }}"
                                                title="Marquer comme arrivé">
                                            <i class="fas fa-check-circle"></i>
                                        </button>
                                        @else
                                        <button class="btn btn-action btn-success" disabled title="Déjà arrivé">
                    ...
                    <!-- Pagination -->
                     @if($planifications->hasPages())
                        <div class="pagination-container">
                            {{ $planifications->links('pagination.modern') }}
                        </div>
                    @endif
                </div>
            </div>
...
// Fonction pour marquer le bateau comme arrivé
    ...
    Swal.fire({
        ...
        html: `
            <div class="text-center">
                <i class="fas fa-ship text-primary fa-3x mb-3"></i>
                <p>Vous êtes sur le point de marquer le bateau comme arrivé :</p>
                <p><strong>"${reference}"</strong></p>
                <p class="text-info">Cette action mettra à jour le statut du bateau.</p>
            </div>
        `,
        icon: 'question',
        ...
        confirmButtonText: '<i class="fas fa-check-circle me-2"></i>Oui, marquer arrivé!',
        ...
        if (result.isConfirmed) {
            ...
            // Utiliser l'URL de la route nommée
            fetch(url, {
                method: 'PUT',
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (row) {
                        ...
                        const statutBadge = row.cells[2].querySelector('.badge');
                        statutBadge.className = 'badge badge-status bg-success';
                        statutBadge.innerHTML = '<i class="fas fa-check me-1"></i>Arrivé';
                        
                        // Mettre à jour l'attribut data-statut
                        row.setAttribute('data-statut', 'arrive');
                    ...
                    Swal.fire({
                        ...
                        html: `
                            <div class="text-center">
                                ...
                                <p>Le bateau a été marqué comme arrivé avec succès.</p>
                            </div>
                        `,
                        icon: 'success',
                        confirmButtonColor: '#28a745',
...
function openBateau(id) {
    ...
    if (confirm('Voulez-vous ouvrir ce bateau pour voir le conteneur associé ?')) {
        ...
        window.location.href = `/admin/bateaux/${id}/conteneur`;
    }
}
