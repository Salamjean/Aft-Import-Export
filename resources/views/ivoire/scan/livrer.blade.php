        ...
        <div class="col-12">
            <div class="card modern-card">
                <div class="card-header modern-header">
                    <div class="header-content">
                        ...
                        <div class="header-text">
                            <h3 class="card-title">Livraison des Colis</h3>
                            <p class="card-subtitle">Scanner les colis pour les livrer aux destinataires</p>
                        </div>
                    </div>
                    <div class="header-actions">
                        <button onclick="openQRScannerLivrer()" class="btn modern-btn text-white" style="background-color:#28a745; margin-right: 10px;">
                            <i class="fas fa-qrcode"></i>
                            Scanner pour Livraison
                        </button>
                        <a href="#" class="btn modern-btn" style="background-color:#6c757d;">
                ...
                <div class="card-body">
                    ...
                    <form method="GET" action="{{ route('ivoire.scan.livrer') }}" class="table-controls">
                        <div class="row">
                            <div class="col-md-4">
                            ...
                            <div class="col-md-8">
                                <div class="filter-group">
                                    ...
                                    <a href="{{ route('ivoire.scan.livrer') }}" class="btn btn-secondary">
                        ...
                        <table class="table modern-table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <!-- Barre de progression pour la livraison -->
                                        @if($item->total_individuels > 0)
                                        @php
                                            $pourcentageLivre = ($item->individuels_livres / $item->total_individuels) * 100;
                                        @endphp
                                        <div class="progress mt-2" style="height: 20px;">
                                            <div class="progress-bar bg-success" role="progressbar" 
                                                 style="width: {{ $pourcentageLivre }}%"
                                                 aria-valuenow="{{ $pourcentageLivre }}" 
                                                 aria-valuemin="0" 
                                                 aria-valuemax="100">
                                        ...
                                        <small class="text-muted">
                                            {{ number_format($pourcentageLivre, 1) }}% livrés
                                        </small>
                                        @else
                                ...
                                <tr>
                                    <td colspan="11" class="text-center">
                                        <div class="no-data-content py-5">
                                            ...
                                            <p class="text-muted">Aucun colis disponible pour la livraison</p>
                                        </div>
                                    </td>
                                </tr>
                                @endforelse
...
/* Styles spécifiques pour la page de déchargement */
.progress {
    border-radius: 10px;
}
...
// Fonction pour ouvrir le scanner QR code pour livraison
function openQRScannerLivrer() {
    console.log("🔓 Ouverture du scanner QR pour livraison...");
        ...
        html: `
            <div class="text-center">
                <div class="alert alert-success">
                    ...
                    Scanner les unités déchargées pour les livrer aux destinataires
                </div>
                <div class="mb-3">
                    <label for="notesLivraison" class="form-label">Notes de Livraison (Optionnel)</label>
                    <textarea class="form-control" id="notesLivraison" rows="2" placeholder="Ex: Livré au destinataire en personne, Signé par..."></textarea>
                </div>
                ...
                <div class="mt-3">
                    ...
                    <div class="btn-group">
                        <button type="button" class="btn btn-warning btn-sm" onclick="switchCameraLivrer()" id="switch-camera-btn-livrer" disabled>
                            <i class="fas fa-camera-rotate"></i> Changer caméra
                        </button>
                        <button type="button" class="btn btn-secondary btn-sm" onclick="stopQRScannerLivrerAndClose()">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <button type="button" class="btn btn-outline-success w-100" onclick="validateManualCodeLivrer()">
                                        Valider
                                    </button>
                                </div>
                            </div>
...
function startQRScannerLivrer() {
    console.log("🚀 Démarrage du scanner livraison...");
    ...
    try {
        ...
        html5QrCode.start(
            ...
            const switchBtn = document.getElementById('switch-camera-btn-livrer');
            if (switchBtn) {
...
function onScanSuccessLivrer(decodedText, decodedResult) {
    ...
    handleScannedQRCodeLivrer(decodedText);
}

// Fonction pour traiter le QR code scanné pour livraison
function handleScannedQRCodeLivrer(qrCode) {
    console.log("🔄 Traitement du QR code pour livraison:", qrCode);
    
    // Récupérer les notes de livraison
    const notesLivraison = document.getElementById('notesLivraison') ? document.getElementById('notesLivraison').value : null;
    ...
    stopQRScannerLivrer().then(() => {
        ...
        showProcessingModalLivrer(qrCode, notesLivraison);
...
// Afficher le modal de traitement pour livraison
function showProcessingModalLivrer(qrCode, notesLivraison) {
    Swal.fire({
        ...
        html: `
            <div class="text-center">
                ...
                <div class="mt-3 p-3 bg-light rounded">
                    ...
                    ${notesLivraison ? `<br><strong>Notes:</strong> ${notesLivraison}` : ''}
                </div>
        ...
        didOpen: () => {
            ...
            processScannedCodeLivrer(qrCode, notesLivraison);
        }
    });
}

// Fonction pour traiter le code scanné avec le serveur pour livraison
function processScannedCodeLivrer(qrCode, notesLivraison) {
    console.log("📡 Envoi au serveur pour livraison:", qrCode, notesLivraison);

    const url = '{{ route("ivoire.scan.qr.livrer") }}';
    ...
    .then(data => {
        ...
        if (data.success) {
            showScanSuccessLivrer(data, qrCode);
        } else {
            showScanErrorLivrer(data, qrCode);
        }
    })
    .catch(error => {
        console.error('❌ Erreur complète:', error);
        showScanConnectionErrorLivrer(error, qrCode);
    });
}

// Afficher le succès du scan pour livraison
function showScanSuccessLivrer(data, qrCode) {
    Swal.fire({
            ...
            <div class="text-center">
                ...
                <div class="alert alert-success">
                    ...
                    <p class="mb-1"><strong>Statut:</strong> 
                        <span class="badge bg-success">${data.unite.nouveau_statut}</span>
                    </p>
                    <p class="mb-1"><strong>Localisation:</strong> ${data.unite.localisation}</p>
                    ${data.unite.notes ? `<p class="mb-1"><strong>Notes:</strong> ${data.unite.notes}</p>` : ''}
                </div>
                ` : ''}
                
                ${data.colis ? `
                <div class="alert alert-info">
                    <h6>📊 Progression de la livraison</h6>
                    <p class="mb-1"><strong>Référence:</strong> ${data.colis.reference_colis}</p>
                    <p class="mb-1"><strong>Progression:</strong> 
                        ${data.colis.unites_livrees}/${data.colis.total_unites} unités livrées
                    </p>
...
// Afficher les erreurs pour la livraison
function showScanErrorLivrer(data, qrCode) {
    Swal.fire({
...
// Afficher les erreurs de connexion pour livraison
function showScanConnectionErrorLivrer(error, qrCode) {
    Swal.fire({
        title: '❌ Erreur de Connexion',
        html: `
            <div class="text-center">
                <i class="fas fa-exclamation-triangle text-danger" style="font-size: 3rem;"></i>
                <p class="mt-3"><strong>Erreur lors de la livraison</strong></p>
                <p class="text-muted">${error.message}</p>
                <div class="mt-3">
                    <button class="btn btn-success me-2" onclick="openQRScannerLivrer()">
...
// Entrée manuelle depuis le scanner pour livraison
function validateManualCodeLivrer() {
    const manualInput = document.getElementById('manualCodeInputLivrer');
    ...
    Swal.close();
    processScannedCodeLivrer(code, notesLivraison);
}

// ============================================================================
// FONCTIONS DU SCANNER SPÉCIFIQUES À LA LIVRAISON
// ============================================================================
...
function stopQRScannerLivrer() {
    return new Promise((resolve, reject) => {
        if (qrScannerLivrer && isScanningLivrer) {
            console.log("🛑 Arrêt du scanner livraison...");
            qrScannerLivrer.stop().then(() => {
                console.log("✅ Scanner livraison arrêté avec succès");
                qrScannerLivrer.clear();
                qrScannerLivrer = null;
                isScanningLivrer = false;
                updateScanStatusLivrer('Scanner arrêté');
                resolve();
            }).catch((error) => {
            ...
            console.log("ℹ️ Scanner livraison déjà arrêté");
            resolve();
        }
    });
}
...
function stopQRScannerLivrerAndClose() {
    stopQRScannerLivrer().then(() => {
        Swal.close();
    });
}
...
function switchCameraLivrer() {
    if (!qrScannerLivrer || !isScanningLivrer) return;
    
    updateScanStatusLivrer('🔄 Changement de caméra...');
    const switchBtn = document.getElementById('switch-camera-btn-livrer');
    if (switchBtn) switchBtn.disabled = true;
...
function openManualInputLivrer() {
    Swal.fire({
        title: '⌨️ Saisie manuelle',
        html: `
            <div class="text-center">
                <i class="fas fa-keyboard text-success" style="font-size: 3rem; margin-bottom: 1rem;"></i>
                <p>Entrez le code du colis manuellement</p>
                <input type="text" id="manualInputLivrer" class="form-control form-control-lg" 
        ...
        preConfirm: () => {
            const code = document.getElementById('manualInputLivrer').value.trim();
            if (!code) {
        ...
        if (result.isConfirmed) {
            processScannedCodeLivrer(result.value, null);
        }
    });
}
...
console.log("✅ Page de livraison des colis chargée avec toutes les fonctionnalités");
</script>
...
function showDetailedStats(colisId) {
    Swal.fire({
        title: '📊 Statistiques Détaillées',
        html: `
            <div class="text-center">
                <i class="fas fa-chart-bar text-info" style="font-size: 3rem; margin-bottom: 1rem;"></i>
                    ...
                    html: `
                        <div class="text-start">
                            ...
                            <div class="progress mb-3" style="height: 25px;">
                                <div class="progress-bar bg-success" style="width: 72%">72% Livrées</div>
                                <div class="progress-bar bg-warning" style="width: 20%">20% En Cours</div>
                                <div class="progress-bar bg-danger" style="width: 8%">8% Retard</div>
                            </div>
                            
                            <div class="alert alert-info">
                                <strong>Performance:</strong> Bonne progression globale
                            </div>
                        </div>
                    `,
                    confirmButtonText: 'Fermer',
