...
<head>
    ...
    <style>
        ...
        .footer-grid {
            ...
            /* Left 60% (Payment Terms), Right 35% (Totals) */
            gap: 5%;
            margin-bottom: 20px;
            align-items: start;
        }
    ...
    <div class="container">
        ...
        <div class="meta-wrapper">
            <table class="meta-table">
                ...
                <tr>
                    ...
                    <td class="meta-value">{{ Auth::guard('agent')->user()->nom ?? 'ADAMA SYLLA' }}</td>
                </tr>
            </table>
        </div>
            ...
            <tbody>
                ...
                @foreach ($colisDetails as $detail)
                    ...
                    <tr>
                        <td>
                            ...
                            <div class="sub-item">{{ $detail['produit'] ?? 'Marchandise' }} -
                                {{ $colis->mode_transit }}</div>
                            <div class="sub-desc">{{ $detail['description'] ?? '' }}</div>
                        </td>
                        <td class="text-right" style="vertical-align: top; padding-top: 10px;">
                            {{ $detail['quantite'] ?? 1 }}</td>
                        <td class="text-right" style="vertical-align: top; padding-top: 10px;">
                            {{ number_format($detail['prix_unitaire'] ?? 0, 0, ',', ' ') }}</td>
                        <td class="text-right" style="vertical-align: top; padding-top: 10px;">
                ...
                @if (isset($colis->prix_service) && $colis->prix_service > 0)
                    ...
                    <tr>
                        ...
                        <td class="text-right" style="vertical-align: top; padding-top: 10px;">
                            {{ number_format($colis->prix_service, 0, ',', ' ') }}</td>
                        <td class="text-right" style="vertical-align: top; padding-top: 10px;">
                            {{ number_format($colis->prix_service, 0, ',', ' ') }}</td>
                    </tr>
                @endif
                ...
                <div class="payment-terms-bar">
                    ...
                    @if ($hasInvestments || $hasInitial)
                        <div style="margin-top: 20px;">
                            <strong style="display:block; margin-bottom:5px; font-size:12px; color: black;">Historique
                                des paiements</strong>
                            <table
                                style="width: 100%; border-collapse: collapse; font-size: 11px; border: 1px solid #ddd;">
                                <thead style="background-color: #f5f5f5;">
                                    <tr>
                                        ...
                                        <th style="border: 1px solid #ddd; padding: 5px; text-align: center;">
                                            Encaissement No.</th>
                                        <th style="border: 1px solid #ddd; padding: 5px; text-align: center;">Montant
                                            payé ({{ $colis->devise ?? 'F.CFA' }})</th>
                                        <th style="border: 1px solid #ddd; padding: 5px; text-align: center;">Mode de
                                            paiement</th>
                                        <th style="border: 1px solid #ddd; padding: 5px; text-align: center;">Référence
                                        </th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    @if ($hasInitial)
                                        <tr>
                                            <td style="border: 1px solid #ddd; padding: 5px; text-align: center;">
                                                {{ $colis->created_at ? $colis->created_at->format('d-m-Y') : '-' }}
                                            </td>
                                            <td style="border: 1px solid #ddd; padding: 5px; text-align: center;">
                                                EN-{{ str_pad($colis->id, 5, '0', STR_PAD_LEFT) }}</td>
                                            <td style="border: 1px solid #ddd; padding: 5px; text-align: center;">
                                                {{ number_format($paiementInitial, 0, ',', ' ') }}</td>
                                            <td style="border: 1px solid #ddd; padding: 5px; text-align: center;">
                                                {{ $colis->methode_paiement ?? 'Espèce' }}</td>
                                            <td style="border: 1px solid #ddd; padding: 5px; text-align: center;">
                                                Paiement Initial</td>
                                        </tr>
                                    @endif
                                    ...
                                    @if ($hasInvestments)
                                        @foreach ($colis->paiements as $paiement)
                                            <tr>
                                                <td style="border: 1px solid #ddd; padding: 5px; text-align: center;">
                                                    {{ $paiement->created_at ? $paiement->created_at->format('d-m-Y') : '-' }}
                                                </td>
                                                <td style="border: 1px solid #ddd; padding: 5px; text-align: center;">
                                                    EN-{{ str_pad($paiement->id, 5, '0', STR_PAD_LEFT) }}</td>
                                                <td style="border: 1px solid #ddd; padding: 5px; text-align: center;">
                                                    {{ number_format($paiement->montant, 0, ',', ' ') }}</td>
                                                <td style="border: 1px solid #ddd; padding: 5px; text-align: center;">
                                                    {{ $paiement->methode_paiement }}</td>
                                                <td style="border: 1px solid #ddd; padding: 5px; text-align: center;">
                                                    {{ $paiement->notes ?? '' }}</td>
                                            </tr>
                                        @endforeach
                ...
                <table class="totals-table">
                    ...
                    <tr>
                        ...
                        <td style="font-weight: bold; text-align: right; padding-right: 15px;">
                            {{ number_format($montantPaye, 2, ',', ' ') }}</td>
                    </tr>
                    <tr>
        ...
        <div class="page-footer">
            <div class="footer-center">
                <strong>7 AVENUE LOUIS BLERIOT LA COURNEUVE 93120 France | Tel. +3397860399 | contacts.aft@gmail.com
                    |</strong><br>
