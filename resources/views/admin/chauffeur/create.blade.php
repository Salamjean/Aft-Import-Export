    ...
    <div class="row justify-content-center">
        <div class="col-12 col-lg-12 col-xl-12">
            ...
            <div class="card shadow-sm mb-5">
                ...
                <div class="card-body">
                    <form action="{{ route('chauffeur.store') }}" method="POST" id="chauffeurForm">
                        @csrf
                                    ...
                                    <tr id="chauffeur-{{ $chauffeur->id }}">
                                        ...
                                        <td class="py-3 px-4 text-center">
                                            <div class="btn-group" role="group">
                                                ...
                                                <form action="{{ route('chauffeur.destroy', $chauffeur->id) }}" 
                        ...
                        <div class="card-footer bg-white border-top">
                            <div class="d-flex justify-content-between align-items-center flex-wrap">
                                <div class="text-muted mb-2 mb-md-0">
                                    Affichage de <strong>{{ $chauffeurs->firstItem() }}</strong> à 
                                    <strong>{{ $chauffeurs->lastItem() }}</strong> sur 
                                    <strong>{{ $chauffeurs->total() }}</strong> chauffeurs
                                </div>
                                
                                <nav aria-label="Pagination">
                                    <ul class="pagination pagination-sm mb-0">
                                        <li class="page-item {{ $chauffeurs->onFirstPage() ? 'disabled' : '' }}">
                                            <a class="page-link" href="{{ $chauffeurs->url(1) }}" aria-label="Première">
                                                <i class="fas fa-angle-double-left"></i>
                                            </a>
                                        </li>
                                        
                                        <li class="page-item {{ $chauffeurs->onFirstPage() ? 'disabled' : '' }}">
                                            <a class="page-link" href="{{ $chauffeurs->previousPageUrl() }}" aria-label="Précédent">
                                                <i class="fas fa-angle-left"></i>
                                            </a>
                                        </li>

                                        @foreach($chauffeurs->getUrlRange(max(1, $chauffeurs->currentPage() - 2), min($chauffeurs->lastPage(), $chauffeurs->currentPage() + 2)) as $page => $url)
                                            <li class="page-item {{ $page == $chauffeurs->currentPage() ? 'active' : '' }}">
                                                <a class="page-link" href="{{ $url }}">{{ $page }}</a>
                                            </li>
                                        @endforeach

                                        <li class="page-item {{ $chauffeurs->hasMorePages() ? '' : 'disabled' }}">
                                            <a class="page-link" href="{{ $chauffeurs->nextPageUrl() }}" aria-label="Suivant">
                                                <i class="fas fa-angle-right"></i>
                                            </a>
                                        </li>
                                        
                                        <li class="page-item {{ $chauffeurs->currentPage() == $chauffeurs->lastPage() ? 'disabled' : '' }}">
                                            <a class="page-link" href="{{ $chauffeurs->url($chauffeurs->lastPage()) }}" aria-label="Dernière">
        ...
        button.addEventListener('click', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                   ...
                   fetch(`{{ route('chauffeur.update', ':id') }}`.replace(':id', chauffeurId), {
                        method: 'POST',
                        body: formData,
                        headers: {
