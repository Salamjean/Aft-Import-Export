...
// Fonction pour télécharger le PDF du conteneur
function downloadConteneurPDF(id, name) {
...
// Version alternative avec fetch (si vous préférez cette approche)
function downloadConteneurPDFWithFetch(id, name) {
    Swal.fire({
    ...
    fetch(`/admin/conteneur/${id}/pdf`)
        ...
        .then(blob => {
            ...
            a.href = url;
            a.download = `conteneur-${name}-${new Date().toISOString().split('T')[0]}.pdf`;
            ...
            Swal.fire({
                ...
                text: `Le rapport du conteneur "${name}" a été généré avec succès`,
                icon: 'success',
                confirmButtonColor: '#f79c14',
                timer: 3000,
                showConfirmButton: false
