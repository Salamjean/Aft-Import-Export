            ...
            <div class="page-header bg-gradient-primary rounded-3 p-4 shadow" style="background: linear-gradient(135deg, #3a913e, #3a913e) !important;">
                <div class="row align-items-center">
                    ...
                    <div class="col-md-4 text-end">
                        <a href="{{ route('recuperation.create') }}" class="btn btn-light btn-lg rounded-pill px-4">
                                        ...
                                        <td style="text-align: center">
                                            <div class="d-flex justify-content-center gap-2">
                                                ...
                                                @if ($recuperation->statut !== 'termine')
                                                    <a href="{{ route('recuperation.edit', $recuperation->id) }}" 
...
async function showDetails(recuperationId) {
    ...
    try {
        const response = await fetch(`/admin/schedule/recuperation/${recuperationId}/details`);
...
function showEtiquettes(recuperationId) {
    ...
    setTimeout(() => {
        window.open(`/admin/recuperation/${recuperationId}/etiquettes`, '_blank');
...
function downloadEtiquettes(recuperationId) {
    ...
    setTimeout(() => {
        window.open(`/admin/recuperation/${recuperationId}/download-etiquettes`, '_blank');
...
async function deleteRecuperation(recuperationId) {
    try {
        ...
        const response = await fetch(`/admin/schedule/recuperation/${recuperationId}`, {
            method: 'POST',
            body: formData,
            headers: {
...
function editRecuperation(recuperationId) {
    ...
    setTimeout(() => {
        window.location.href = `/admin/recuperation/${recuperationId}/edit`;
    }, 1100);
}
