            ...
            <div class="card modern-card">
                <div class="card-header modern-header">
                        ...
                        <div class="header-text">
                            <h3 class="card-title">Vous pouvez suivre vos colis</h3>
                            <p class="card-subtitle">Liste de tous vos colis enregistrés</p>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                   ...
                   <!-- Filtres et recherche -->
                    <form method="GET" action="{{ route('user.colis.index') }}" class="table-controls">
                        <div class="row">
                            <div class="col-md-4">
                            ...
                            <div class="col-md-8">
                                <div class="filter-group">
                                    <!-- Changer "status" en "statut" pour correspondre au controller -->
                                    <select class="form-control modern-select" name="statut">
                                        <option value="">Tous les statuts</option>
                                        <option value="valide" {{ request('statut') == 'valide' ? 'selected' : '' }}>Validé</option>
                                        <option value="charge" {{ request('statut') == 'charge' ? 'selected' : '' }}>Chargé</option>
                                        <option value="entrepot" {{ request('statut') == 'entrepot' ? 'selected' : '' }}>En Entrepôt</option>
                                        <option value="decharge" {{ request('statut') == 'decharge' ? 'selected' : '' }}>Déchargé</option>
                                    ...
                                    <button type="submit" class="btn text-white" style="background-color: #0e914b">
                                        <i class="fas fa-filter"></i> Filtrer
                                    </button>
                                    <a href="{{ route('user.colis.index') }}" class="btn btn-secondary">
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        ...
                                        <span class="status-badge status-{{ $item->statut }}">
                                            {{ $labels[$item->statut] ?? $item->statut }}
                                        </span>
                                    </td>
                                    <td class="text-center">
                                    ...
                                    <td class="text-center">
                                        <div class="action-buttons">
                                            ...
                                            {{-- <a href="{{ route('user.colis.edit', $item->id) }}" class="btn-action btn-edit" title="Modifier">
                    ...
                    <div class="text-start">
                        ...
                        <div class="card mb-3">
                            <div class="card-header bg-light d-flex justify-content-between align-items-center">
                                ...
                                <span class="badge bg-primary text-white">${data.total_individuels || 0} unité(s)</span>
                            </div>
                            <div class="card-body">
...
function generateStatutsIndividuelsHTML(statutsIndividuels, compteurStatuts) {
    ...
    statutsArray.forEach(statut => {
        html += `
            <tr>
                <td class="text-center"><strong>${statut.produit}</strong></td>
                <td class="text-center">
...
function getStatusBadgeClass(statut) {
    const classes = {
        ...
        'entrepot': 'status-entrepot',
        'decharge': 'status-decharge',
        'livre': 'status-livre',
...
function processPayment(colisId, paymentData, reference) {
    ...
    fetch(`/user/parcel/${colisId}/paiement`, {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Content-Type': 'application/json',
...
function generateDocument(colisId, type, action) {
    ...
    switch(type) {
        case 'etiquette':
            url = `/user/parcel/${colisId}/etiquettes?action=${action}`;
            break;
        case 'facture':
            url = `/user/parcel/${colisId}/facture?action=${action}`;
            break;
        case 'bon_livraison':
            url = `/user/parcel/${colisId}/bon-livraison?action=${action}`;
            break;
    }
...
function downloadAllColisPDF() {
    ...
    let url = '/user/parcel/export/pdf?' + searchParams.toString();
